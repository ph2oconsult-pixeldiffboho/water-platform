...
""")
        ...
        with st.expander("**Input Validation — runs automatically**"):
            st.markdown("""
**When it runs:** Every time the WaterPoint Intelligence panel loads on Page 04.

**What it checks:**
- **Physical plausibility** — impossible input combinations (peak < average flow,
  NH₄ > TN load, MLSS out of range). These are Critical flags.
- **Ratio anomalies** — COD:TKN signals (carbon-limited < 8, trade waste > 13),
  NH₄:TKN outside domestic range, BOD:TSS Healesville-class mismatch.
- **Missing fields** — six required fields; five optional but high-value fields.
- **Data confidence** — scored as High / Acceptable / Low / Very Low based on
  the number and severity of issues found.
- **VOI assessment** — identifies which gaps carry High VOI (peak flow, aeration
  audit, TN target, fractionation, trade waste characterisation).
- **Governing condition** — identifies whether the design is governed by
  dry weather ADWF, peak wet weather, or a seasonal event.

**What to do with flags:**
- 🔴 Critical flags must be resolved before relying on outputs — they indicate
  physically impossible inputs or missing required fields.
- 🟡 Warning flags do not block analysis but must be understood before
  process selection is finalised.
- 🔍 High VOI fields shown in the panel indicate the highest-return next investigation.

See the **Input Reference** tab for the full list of validation rules (IV-01 to IV-30).
""")
...
""")
        ...
        st.markdown(
            "WaterPoint runs an automatic input validation check every time the WaterPoint "
            "Intelligence panel loads. This implements **Parts 1–3 of the nine-part decision "
            "framework** — IDE integrity, data confidence scoring, and governing condition "
            "identification — directly against your input data."
        )
        st.info(
            "**Where to find it:** The validation panel appears above all WaterPoint output "
            "sections on the Results page, labelled **📋 Input Validation**. "
            "It expands automatically when Critical flags are present."
        )
...
|---|---|---|
| 🔴 **Critical** | Physically impossible input or missing required field | Resolve before relying on outputs |
| 🟡 **Warning** | Ratio or range anomaly — not impossible but requires understanding | Investigate before detailed design |
| ℹ️ **Info** | Optional field missing or parameter at the edge of typical range | Note and proceed |

**Critical flags** set `safe_for_analysis = False` and are injected into the credibility layer
as consistency flags. If data confidence is Very Low, `ready_for_client` is also demoted.
""")

        with st.expander("**Validation rules (IV-01 to IV-30)**"):
            st.markdown("""
**Physical plausibility rules (Critical)**

| Rule | Field | Check |
|---|---|---|
| IV-01 | peak_flow_mld | Peak flow < average flow — physically impossible |
| IV-02 | peak_flow_mld | Peak:ADWF ratio > 10× — implausible for reticulated sewer |
| IV-03 | peak_flow_mld | Peak:ADWF ratio > 5× — very high I/I signal (Warning) |
| IV-04 | current_load.nh4_kg_d | NH₄ load > TN load — chemically impossible |
| IV-05 | effluent_nh4_mg_l | NH₄ effluent > TN effluent — component exceeds whole |
| IV-06 | mlss_mgL | MLSS < 1,000 mg/L — below plausible activated sludge range |
| IV-07 | mlss_mgL | MLSS > 8,000 mg/L — above plausible range |
| IV-08 | current_load.bod_kg_d | Implied BOD concentration < 50 mg/L — dilution or mismatch |
| IV-09 | o2_demand_kg_day | O₂/kWh ratio outside 0.5–3.0 kg O₂/kWh — units check |

**Ratio-based warnings (Warning)**

| Rule | Field | Check |
|---|---|---|
| IV-10 | cod_to_n_ratio | COD:TKN < 8 — carbon-limited denitrification signal |
| IV-11 | cod_to_n_ratio | COD:TKN 8–9 — at denitrification threshold; Fbs-sensitive |
| IV-12 | cod_to_n_ratio | COD:TKN > 13 — trade waste influence likely |
| IV-13 | current_load.nh4_kg_d | NH₄:TKN < 0.50 — high organic N; trade waste protein |
| IV-14 | current_load.nh4_kg_d | NH₄:TKN 0.50–0.60 — below domestic range lower bound |
| IV-15 | current_load.nh4_kg_d | NH₄:TKN > 0.75 — above domestic range; septic influence |
| IV-16 | BOD:TSS ratio | BOD:TSS > 2.5 — Healesville-class pattern; diurnal mismatch risk |
| IV-16b | BOD:TSS ratio | BOD:TSS < 0.5 — high inorganic or particulate industrial load |

**Missing field checks**

| Rule | Field | Severity |
|---|---|---|
| IV-17 | average_flow_mld | Critical |
| IV-18 | technology_code | Critical |
| IV-19 | effluent_tn_mg_l | Critical |
| IV-20 | tn_target_mg_l | Critical |
| IV-21 | current_load.bod_kg_d | Critical |
| IV-22 | current_load.tn_kg_d | Critical |
| IV-23 | o2_demand_kg_day | Info |
| IV-24 | aeration_kwh_day | Info |
| IV-25 | peak_flow_mld | Info |
| IV-26 | clarifier_area_m2 | Info |
| IV-27 | reactor_volume_m3 | Info |
""")

        with st.expander("**Data confidence levels (Part 2 of Decision Framework)**"):
            st.markdown("""
The validation layer assigns one of four confidence levels to the overall input dataset.
This mirrors the confidence levels documented in the Decision Framework tab.

| Level | Trigger condition | Downstream effect |
|---|---|---|
| 🟢 **High** | No warnings or missing fields | No downstream adjustment |
| 🟡 **Acceptable** | 1 warning or 1 missing optional field | Noted; no adjustment |
| 🔴 **Low** | ≥1 missing critical field OR ≥4 warnings | Credibility layer notes |
| 🔴 **Very Low** | ≥1 Critical flag OR ≥3 missing critical fields | `ready_for_client` demoted to False |

**The high-volume / low-confidence paradox** applies here too:
fields with abundant data but low representativeness (e.g. flow data that doesn't
capture the peak event) will not trigger a validation flag because the flag system
operates on structured input fields, not raw data quality. The VOI assessment and
the Decision Framework tab address this dimension.
""")

        with st.expander("**Value of Information (VOI) assessment (Part 2)**"):
            st.markdown("""
The validation layer assesses which missing or uncertain fields carry High VOI —
meaning more data would change process selection, sizing, or risk.

| Field | VOI | Rationale |
|---|---|---|
| peak_flow_mld (missing) | High | Cannot apply binding constraint test to hydraulic options |
| peak_flow_mld (ratio ≥ 3×) | High | Wet weather likely governing condition — clarifier assessment required |
| cod_to_n_ratio > 13 + fractionation | High | Trade waste may overstate effective denitrification carbon |
| o2_demand_kg_day + aeration_kwh_day (both missing) | High | Cannot assess blower headroom — MABR/IFAS selection is blind |
| tn_target_mg_l (missing) | High | TN 5 mg/L vs 10 mg/L drives fundamentally different process selection |
| Trade waste characterisation | High | Seasonal peak likely not captured in current load data |
| Diurnal profiling (BOD:TSS > 2.0) | Moderate | N/COD peak mismatch — affects control strategy, not process selection |
| o2_demand_kg_day (aeration_kwh_day present) | Moderate | Improves aeration efficiency precision; not selection-changing |

High VOI fields are displayed in the validation panel to guide the next investigation steps.
""")

        with st.expander("**Governing condition (Part 3)**"):
            st.markdown("""
The validation layer identifies the governing design condition from the flow data.
This is displayed at the top of the validation panel.

| Condition | Trigger | Implication |
|---|---|---|
| **Peak wet weather** | Peak:ADWF ≥ 3× | Clarifier hydraulic performance is the primary constraint assessment |
| **Moderate wet weather / diurnal peak** | Peak:ADWF 2–3× | Both wet weather hydraulic and dry weather biological performance matter |
| **Seasonal trade waste event** | COD:TKN > 13, no high peak ratio | Vintage or food processing peak likely governing — not captured in data |
| **Dry weather ADWF** | None of the above | Standard biological design basis; confirm against compliance failure history |
| **Unknown** | No flow data | Governing condition cannot be determined — provide flow data |

**Rule: Always design for the governing condition — not the average.**
The governing condition identification is the mechanism that enforces this rule automatically.
""")
